---
title: Deployment
description: Deploy Binah to Vercel, Docker, or your own host.
---

Binah is a standard Next.js application. It runs anywhere Next.js runs.

## Vercel (one click)

The fastest path is the Deploy button in the [repository README](https://github.com/Lineker-star/binah). Fork the repository and enter an API key for at least one LLM when prompted.
...
The default Compose deployment starts the Binah app and mounts the `binah-data` volume. Configure other providers and features as needed; see [Configuration](./configuration.mdx).
...
Inside a Docker container, `localhost` refers to the container itself. If Ollama, Lemonade, VoxCPM, or ComfyUI runs on the host, use `host.docker.internal`, for example `http://host.docker.internal:11434/v1` or `http://host.docker.internal:8188`. Linux Docker usually also requires `extra_hosts: ["host.docker.internal:host-gateway"]` on the `binah` service. Because ComfyUI is not currently a server-managed provider, production deployments must also set `ALLOW_LOCAL_NETWORKS=true`, or the SSRF guard will reject the address.
...
The repository's `server-persistence` profile starts Binah and PostgreSQL in two containers. The persistence HTTP API is embedded in Binah; no separate persistence service is required.
...
PostgreSQL data is stored in the `binah-postgres` volume. `PERSISTENCE_POSTGRES_PASSWORD` sets the password only when the database directory is first initialized. Changing the environment variable later does not automatically change the existing database user's password.
...
Compose connects Binah to the rendering service through `RENDER_SERVICE_URL`. If the profile is disabled or the rendering service is unavailable, export falls back to downloading the project ZIP for rendering with the local CLI. The rendering service uses an isolated network and requires the `NET_ADMIN` capability at startup. See [`render-service/README.md`](https://github.com/Lineker-star/binah/blob/main/render-service/README.md) for more limitations and standalone deployment instructions.
