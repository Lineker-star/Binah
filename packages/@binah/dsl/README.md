...
The **contract keystone** of the MAIC SDK family. `@binah/dsl` is *pure spec* — the
...
package can depend on `@binah/dsl` without pulling in junk.

## Dependency arrows (acyclic)

```
@binah/dsl       ->  (nothing)
@binah/renderer  ->  @binah/dsl
@binah/importer  ->  @binah/dsl
@binah/exporter  ->  @binah/dsl     (reserved, future)
```

`@binah/dsl` is the only package everything else depends on, and it depends on
nothing.
...
```ts
import type { Slide, PPTElement, Action } from '@binah/dsl';
import { isTextElement, DSL_VERSION, SYNC_ACTIONS } from '@binah/dsl';
```
...
1. **JSON Schema artifacts (cross-language mirror)** — `Stage`, the default
   ...
   ```ts
   import stageSchema from '@binah/dsl/schema/stage.schema.json' with { type: 'json' };
   import sceneSchema from '@binah/dsl/schema/scene.schema.json' with { type: 'json' };
   import actionSchema from '@binah/dsl/schema/action.schema.json' with { type: 'json' };
...
2. **Pure validators (in-process boundary)** — `validate*` are hand-written,
   ...
   ```ts
   import { validateStage, validateScene, validateAction } from '@binah/dsl';
...
3. **Pure normalizers (repair boundary)** — where `validate*` *reports* on a
   ...
   ```ts
   import { normalizeElement, normalizeScene, ELEMENT_DEFAULTS } from '@binah/dsl';
...
```ts
import { DSL_VERSION, migrate, dslVersionOf, needsMigration } from '@binah/dsl';
...
outside the document, per learner, through a `RuntimeStore` (`@binah/storage`).
...
Both consumers are now wired to `@binah/dsl` and no longer vendor their own copy
of the slide types:

- **`@binah/importer`**: imports all slide types from `@binah/dsl`; vendored
  `binah/types/slides.ts` deleted. The importer emits complete DSL `Slide`
  objects directly (the old partial "draft slide" + post-fill step is gone).
- **`@binah/renderer`**: imports all slide types from `@binah/dsl`; vendored
  `types/slides.ts` deleted. `@binah/dsl` is a regular dependency, kept external
  in the rollup build so consumers share one copy. The public
  `@binah/renderer/types` surface now re-exports the DSL types.

### Roadmap

- [x] Wire `@binah/importer` to import types from `@binah/dsl` (vendored copy deleted).
- [x] Wire `@binah/renderer` to import types from `@binah/dsl` (vendored copy deleted).
...
- [ ] Reserve `@binah/exporter` as the 4th family member.
...
```ts
import type { Action, InteractiveContent, PBLContent, Scene, SceneContent, WidgetConfigBase } from '@binah/dsl';
...
```bash
pnpm --filter @binah/dsl build         # -> dist/ (index.js, index.d.ts, …) + dist/schema/*.json
pnpm --filter @binah/dsl build:schema  # regenerate only dist/schema/*.json
pnpm --filter @binah/dsl typecheck
pnpm --filter @binah/dsl test
```

## License

MIT, matching the rest of the family (`@binah/dsl`, `@binah/importer`,
`@binah/renderer`) and the Binah root, so the license policy is uniform
across the SDK.
