...
function assertStorableScene(scene: SceneLike, stageId: string): void {
  ...
  if (typeof s.id !== 'string') {
    throw new Error(`@binah/storage: scene id must be a string, got ${JSON.stringify(s.id)}`);
  }
  if (s.stageId !== stageId) {
    throw new Error(
      `@binah/storage: scene ${JSON.stringify(s.id)} has stageId ` +
        `${JSON.stringify(s.stageId)} but belongs to document ${JSON.stringify(stageId)}`,
    );
  }
  if (typeof s.order !== 'number' || !Number.isFinite(s.order)) {
    throw new Error(
      `@binah/storage: scene ${JSON.stringify(s.id)} order must be a finite number, got ` +
  ...
  async saveDocument(doc: MaicDocument<TScene, TStage>): Promise<void> {
    ...
    for (const scene of normalized.scenes) {
      ...
      if (seen.has(scene.id)) {
        throw new Error(
          `@binah/storage: duplicate scene id ${JSON.stringify(scene.id)} in document ${JSON.stringify(stageId)}`,
        );
      }
      seen.add(scene.id);
    }
    ...
    await this.txRun([STAGES], 'readwrite', async (tx) => {
      ...
      if (dslVersionOf(stageRow) !== DSL_VERSION) {
        throw new DocumentVersionError(
          ...
          `@binah/storage: cannot putStage into document ${JSON.stringify(stageId)} at DSL ` +
    ...
    await this.txRun([STAGES, SCENES], 'readwrite', async (tx) => {
      ...
      if (dslVersionOf(stageRow) !== DSL_VERSION) {
        throw new DocumentVersionError(
          ...
          'not-current',
          stageRow[DSL_VERSION_KEY],
          `@binah/storage: cannot putScene into document ${JSON.stringify(stageId)} at DSL ` +
            `version ${JSON.stringify(dslVersionOf(stageRow))} — load and save it to bring it ` +
            `to ${DSL_VERSION} first`,
        );
      }
    ...
    await this.txRun([STAGES, SCENES], 'readwrite', async (tx) => {
      ...
      if (dslVersionOf(stageRow) !== DSL_VERSION) {
        throw new DocumentVersionError(
          ...
          'not-current',
          stageRow[DSL_VERSION_KEY],
          `@binah/storage: cannot deleteScene from document ${JSON.stringify(stageId)} at DSL ` +
            `version ${JSON.stringify(dslVersionOf(stageRow))} — load and save it to bring it ` +
            `to ${DSL_VERSION} first`,
        );
      }
