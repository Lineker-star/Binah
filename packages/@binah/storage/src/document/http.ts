import { migrate, validateScene, validateStage } from '@binah/dsl';
...
> implements DocumentStore<TScene, TStage> {
  ...
  constructor(options: HttpDocumentStoreOptions) {
    if (options.baseUrl === '') {
      throw new Error('@binah/storage: HttpDocumentStore baseUrl must be non-empty');
    }
    ...
    if (typeof selectedFetch !== 'function') {
      throw new Error('@binah/storage: HttpDocumentStore requires a fetch implementation');
    }
    ...
    if (!response.ok) {
      ...
      const message =
        typeof errorBody?.error?.message === 'string'
          ...
          : `@binah/storage: DocumentStore HTTP request failed with status ${response.status}`;
      if (
        response.status === 409 &&
        code === 'FUTURE_VERSION' &&
  ...
  async listDocuments(): Promise<DocumentSummary[]> {
    ...
    if (!Array.isArray(summaries)) {
      throw new HttpDocumentStoreError(
        200,
        'MALFORMED_RESPONSE',
        '@binah/storage: DocumentStore HTTP listDocuments response must be an array',
      );
    }
