...
function registryConfigurationFailure(problem: string): Error {
  return new Error(`@binah/storage: ${problem}`);
}

function registryFailure(operation: string): Error {
  return new Error(`@binah/storage: asset registry ${operation} failed`);
}
...
function encodeMeta(meta: AssetMeta): string {
  ...
  try {
    ...
    throw new Error('@binah/storage: asset metadata is not JSON-serializable');
  }
}
...
export class PgAssetStore implements AssetStore {
  ...
  constructor(
    ...
    if (typeof options?.withTransaction !== 'function') {
      throw new Error(
        '@binah/storage: withTransaction is required and must pin a fresh connection and transaction for every call',
      );
    }
    if (!options.byteStore) {
      throw new Error('@binah/storage: byteStore is required for PgAssetStore');
    }
    if (
      options.quotaBytes !== undefined &&
      (!Number.isSafeInteger(options.quotaBytes) || options.quotaBytes < 0)
    ) {
      throw new Error('@binah/storage: quotaBytes must be a non-negative safe integer');
    }
