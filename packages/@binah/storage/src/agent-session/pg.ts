...
const OWNER_EVENT_TYPE_CONSTRAINT_V2_SENTINEL = '__BINAH_OWNER_EVENT_TYPE_KNOWN_V2__';
const TITLE_STATE_CONSTRAINT = 'agent_sessions_title_state_known';
const TITLE_STATE_CONSTRAINT_SENTINEL = '__BINAH_SESSION_TITLE_STATE_KNOWN__';
...
function quoteIdentifier(identifier: string): string {
  if (!/^[a-z_][a-z0-9_]*$/.test(identifier)) {
    throw new Error(
      `@binah/storage: invalid agent-session table name ${JSON.stringify(identifier)}`,
    );
  }
  return `"${identifier}"`;
}
    ...
    AgentSessionUrlStore
{
  ...
  async appendUserMessage(
    ...
    return this.transaction(async (tx) => {
      const parent = await this.lockForAppend(tx, sessionId, null);
      if (!parent)
        throw new Error(`@binah/storage: unknown session ${JSON.stringify(sessionId)}`);
      return this.insertEvent(tx, sessionId, {
        ts: this.clock(),
        attempt: 0,
