...
persisting app state, depending only on [`@binah/dsl`](../dsl).
...
| `StorageProvider` (from `@binah/dsl`) | the asset seam: `put(blob) → ref`, `resolve(ref) → url`, `remove(ref)` | `BrowserAssetStore` over IndexedDB (`assets` registry + `blobs`) + object URLs |
...
- [x] `StorageProvider` (in `@binah/dsl`) + browser asset registry: allocated
