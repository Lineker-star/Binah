...
Composable slide-editing package for Binah.

- `@binah/editor/core`: document operations, transactions, and undo/redo history.
- `@binah/editor/react`: editable slide interaction surface and rich-text editors.
- `@binah/editor/ui`: editor toolbars, insertion controls, and context menus.

## Dependencies

```text
@binah/editor
├─> @binah/renderer   (reuses the read-only slide renderer)
└─> @binah/dsl        (document and element data contracts)
```

`@binah/renderer` does not depend on `@binah/editor`, so the package boundary remains
...
`onTransaction` sink. `@binah/editor` owns built-in element adapters, insertion defaults,
...
```bash
pnpm add @binah/editor @binah/dsl @binah/renderer \
...
`node_modules/@binah/renderer/dist/**/*.{js,cjs}` as described in the
[`@binah/renderer` setup](https://www.npmjs.com/package/@binah/renderer#tailwind-4-setup).
...
import { applyEditorTransaction, createEditorHistory, type EditorTransaction } from '@binah/editor/core';
import { EMPTY_SELECTION, type Selection } from '@binah/editor/react';
import { EditableSlideCanvasWithUI, type EditorInsertItem } from '@binah/editor/ui';
