/**
 * High-level entry: parsed pptxtojson(-pro) JSON → Binah canvas `Slide[]`.
 *
 ...
 * whole import either.
 */
import { normalizeSlideWith, type Slide } from '@binah/dsl';
...
/**
 * Convert a pre-parsed pptxtojson(-pro) `Output` JSON into Binah slides.
 *
...
const normalizeImportedSlide = normalizeSlideWith({
  ...
  onDropped: (_element, error) => {
    console.warn(
      `[@binah/importer] dropping element that fails DSL normalization: ${
...
/**
 * Parse a .pptx file and convert it into Binah canvas slides.
 *
