...
> `@binah/renderer` is the read-only canvas package. Editing lives in the
> separate `@binah/editor` package, which depends on this renderer.

## Migrating from 0.0.x

Version `0.1.0` removes the experimental `@binah/renderer/editing` subpath.
Install `@binah/editor` and migrate editing imports to its explicit layers:

```ts
import { EditableSlideCanvas } from '@binah/editor/react';
import { EditableSlideCanvasWithUI } from '@binah/editor/ui';
```

Read-only rendering imports from `@binah/renderer` are unchanged.

## Install

```bash
pnpm add @binah/renderer
# or
npm install @binah/renderer
```
...
```tsx
import { SlideCanvas, type Slide } from '@binah/renderer';
...
```tsx
import { SlideRendererProvider, SlideCanvas, useSlideContext } from '@binah/renderer';
...
### Granular components — `@binah/renderer/elements`
...
export default {
  content: [
    './src/**/*.{ts,tsx}',
    './node_modules/@binah/renderer/dist/**/*.{js,cjs}',
  ],
};
```
...
[`@binah/importer`](../importer) converts `.pptx` files to the same `Slide[]` shape, so you can do `.pptx → @binah/renderer` end-to-end.
