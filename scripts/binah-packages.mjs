...
/**
 * The one list of owned, publishable @binah packages.
 *
 ...
 */
export const BINAH_PACKAGES = ['dsl', 'generation', 'storage', 'renderer', 'editor', 'importer'];

/** The packages that depend on other owned packages, and on which ones. */
export const INTERNAL_DEPENDENTS = {
  generation: ['@binah/dsl'],
  storage: ['@binah/dsl'],
  renderer: ['@binah/dsl'],
  editor: ['@binah/dsl', '@binah/renderer'],
  importer: ['@binah/dsl'],
};

export const repositoryRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..');

export const PACKAGES_DIRECTORY = join(repositoryRoot, 'packages/@binah');
...
export function assertPackageListIsComplete() {
  ...
  const onDisk = readdirSync(PACKAGES_DIRECTORY, { withFileTypes: true })
    ...
    .sort();
  const listed = [...BINAH_PACKAGES].sort();

  for (const name of onDisk) {
    if (!listed.includes(name)) {
      problems.push(
        `packages/@binah/${name} exists but is not in BINAH_PACKAGES, so every check ` +
  ...
  for (const name of listed) {
    if (!onDisk.includes(name)) {
      problems.push(`BINAH_PACKAGES lists ${name}, but packages/@binah/${name} is gone.`);
    }
  }
...
function describeSetDifference(expected, observed, subject) {
  ...
  for (const name of observed) {
    if (!expected.includes(name)) {
      problems.push(`${subject} names ${name}, which is not in BINAH_PACKAGES.`);
    }
  }
  return problems;
}
 ...
 * (the build, pack, publish and tag loops). `--filter "@binah/<name>"` is
...
function crossCheckPublishWorkflow(workflow) {
  ...
  // up `packages/@binah/$pkg/package.json` inside the shell loops, where
  ...
  if (!pathsBlock) {
    ...
    const triggerPaths = [
      ...pathsBlock[1].matchAll(/packages\/@binah\/([^/"'\s]+)\/package\.json/g),
    ].map((match) => match[1]);
    problems.push(
  ...
  const filtered = new Set(
    [...source.matchAll(/--filter\s+"?@binah\/([^"\s]+)"?/g)].map((match) => match[1]),
  );
  for (const name of BINAH_PACKAGES) {
    if (!filtered.has(name)) {
      problems.push(
        `${PUBLISH_WORKFLOW} never passes --filter "@binah/${name}", so it is never built ` +
          'or tested on the release path.',
      );
    }
  }
  for (const name of filtered) {
    if (!BINAH_PACKAGES.includes(name)) {
      problems.push(
        `${PUBLISH_WORKFLOW} filters @binah/${name}, which is not in BINAH_PACKAGES.`,
      );
    }
  }
