...
/**
 * Pure quiz question-rendering/answer-selection UI — extracted from
 * components/scene-renderers/quiz-view.tsx so it can be reused by any
 * quiz-taking surface, not just one embedded in a classroom scene. None of
 * these take a sceneId/stageId; the draft-recovery/attempt-runtime
 * persistence layer (lib/quiz/runtime.ts, lib/quiz/view-state.ts) stays
 * behind in quiz-view.tsx, since that layer IS genuinely scene/stage-keyed
 * and doesn't generalize the same way.
 */
...
export const QuizMathText = memo(function QuizMathText({
  text,
  className,
...
export function QuizCover({
  ...
  totalPoints: number;
  onStart: () => void;
  /** Defaults to the in-scene quiz's generic copy; a standalone surface
   *  (Continuous Assessment, the future Exam) passes its own. */
  title?: string;
  subtitle?: string;
}) {
  const { t } = useI18n();
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center gap-4 relative overflow-hidden">
      ...
      >
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{subtitle ?? t('quiz.subtitle')}</p>
      </motion.div>
