...
const publishScript = resolve(repositoryRoot, '.github/scripts/publish-binah-skill.sh');
...
function runPublish(options: RunOptions = {}) {
  ...
  Object.assign(env, {
    ...
    SOURCE_REPO: options.sourceRepo ?? 'Lineker-star/binah',
...
describe('publish-binah-skill shell contract', () => {
  ...
  it('rejects unsupported arguments', () => {
    ...
    expectFailure(usage.result, 'Usage: publish-binah-skill.sh [--dry-run]');
  ...
  it.each([
    ...
    expectFailure(result, 'Usage: publish-binah-skill.sh [--dry-run]');
