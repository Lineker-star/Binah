---
name: binah
description: Binah assistant for setting up, generating, and extending Binah. Use when the user wants to use Binah, generate a multi-agent interactive classroom, or build on / extend / customize Binah and its @binah/* SDK (secondary development, 二开) — covers local setup, startup modes, provider keys, classroom generation, and secondary development (forking, providers/storage/themes, routes, or the renderer/editor).
user-invocable: true
...
- Do not assume the OpenClaw agent's own model or API key will be reused by Binah.
- Binah classroom generation uses Binah server-side provider config.
- This skill must not rely on any request-time model or provider overrides.
- Only Binah server-side config files may control provider selection and defaults.
...
Ask the user how they want to use Binah:

1. **Run locally** (recommended for quick start) — Clone the repo, configure provider keys, and run on your machine.
2. **Extend or build on Binah (二次开发)** — Fork the repo and customize the product, or consume the `@binah/*` SDK to build something new.

If the user chooses local mode, proceed to phase 1 as usual.
If the user chooses to extend/build on Binah, load [references/extend.md](references/extend.md) and skip the setup/generation phases.

### 1. Clone Or Reuse Existing Repo

Load [references/clone.md](references/clone.md).

Use this when the user has not installed Binah yet or when you need to confirm which local checkout to use.
...
### 4. Start And Verify Binah

After the user has chosen a startup mode and configured keys, start Binah using the chosen method, then verify the service with `GET {url}/api/health`.
