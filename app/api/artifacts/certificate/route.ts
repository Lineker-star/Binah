 ...
 * signed download URL for a completed learning experience's plain
 * Certificate of Completion. Takes either a `courseId` (Structured Course)
 * or a `sessionId` (ad-hoc single-prompt session with no courses row). A
 * certificate is naturally a singleton per course or session: if one
 * already exists, this signs and returns the existing storage object
 * instead of creating a duplicate generated_artifacts row/Storage object.
 *
 * Unconditional — no lesson-count threshold, no grade/score/skills. Every
 * completed course or session gets this one regardless of size. The
 * richer "Certificate of Excellence" (grade, overall_score,
 * skills_acquired, serial_code — populating the `certificates` table) is
 * a separate, ADDITIONAL certificate reserved for courses over 10 lessons
 * — see app/api/artifacts/certificate-excellence/route.ts. The two are
 * kept as distinct generated_artifacts rows (artifact_type='certificate'
 * here vs 'certificate_excellence' there) so they're never confused in
 * history/downloads; a learner who qualifies gets both, independently.
 *
